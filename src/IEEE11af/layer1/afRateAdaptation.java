 ...
 * @author jyul5586
 */
public class afRateAdaptation {

    public void event_increase_phy_mode_ind(JEEvent anEvent, afPhy phy) {
        ...
        } else if (phy.equals("BPSK34")) {
            phy.setCurrentPhyMCS("QPSK12");
        } else if (phy.equals("BPSK12")) {
            phy.setCurrentPhyMCS("BPSK34");
        } else {
            System.out.print("\n Incorrect PhyMCS");
        }
    }

    public void event_reduce_phy_mode_ind(JEEvent anEvent, afPhy phy) {
        switch (phy.toString()) {
            case "256QAM56":
                phy.setCurrentPhyMCS("256QAM34");
                break;
