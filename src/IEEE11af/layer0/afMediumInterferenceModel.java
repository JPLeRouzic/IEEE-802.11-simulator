...
public class afMediumInterferenceModel extends JEEventHandler implements WirelessMedium {
	...
	final List<afPhy> allPhys;

	// lookup table for attenuation factors
        // key, (key, value)
        // station, (station, attenuation)
        
        public afAttenuationTable attnTable ;

	final Map<Integer, WirelessChannel> availableChannels;
        
        public afPathLoss palo = new afPathLoss() ;

	public afMediumInterferenceModel(final JEEventScheduler aScheduler, final Random aGenerator, final Node aTopLevelNode) {
...
//		acAttenuationTable = new HashMap<Integer, Map<Integer, Double>>();
                attnTable = new afAttenuationTable();
                
		allPhys = new ArrayList<afPhy>();
	...
	private void transmitStart(final JEEvent anEvent) {
		...
		afPhy aTxPhy = (afPhy) anEvent.getParameterList().elementAt(0);
		...
		Map<Integer, Double> neighbors = afAttenuationTable.get(aTxPhy.getMac().getMacAddress());
		...
		List<afPhy> channelPhys = chan.getPhyList();
		int size = channelPhys.size();
		for (int i = 0; i < size; i++) {
			afPhy rxPhy = channelPhys.get(i);
	...
	private void transmitEnd(final JEEvent anEvent) {
		...
		for (Integer rxStation : transmission.getRxStations()) {
			...
			JE802Phy rxPhy = null;
			// search for rxPhy
			for (afPhy aRxPhy : channel.getPhyList()) {
	...
	private void switchChannel(JEEvent anEvent) {
		afPhy phy = (afPhy) anEvent.getParameterList().get(0);
	...
	private double calculateInterference(final afPhy atPhy, final JE802Phy txPhy, final JE802ChannelEntry chan) {
	...
	private void registerPhy(final JEEvent anEvent) {
		afPhy newPhy = (afPhy) anEvent.getParameterList().elementAt(0);
	...
	private void locationUpdate(JEEvent anEvent) {
		afPhy updatePhy = (afPhy) anEvent.getParameterList().get(0);
	...
	protected class JE802ChannelEntry {
		...
		private final List<afPhy> phyList;
		...
		public List<afPhy> getPhyList() {
			return phyList;
		}

		public void addPhy(final afPhy phy) {
			phyList.add(phy);
			for (JE802MediumTxRecord rec : transmittingPhys) {
		...
		public JE802MediumTxRecord addTransmittingPhy(final afPhy txPhy, final JETime txEnd) {
			JE802MediumTxRecord txRec = new JE802MediumTxRecord(txPhy, txEnd);
			Map<Integer, Double> neighbors = afAttenuationTable.get(txPhy.getMac().getMacAddress());
			Set<Integer> rxStations = new HashSet<Integer>();
			for (afPhy addr : phyList) {
	...
	public class JE802MediumTxRecord {
		...
		public void addRxStation(afPhy phy, double interference) {
    ...
    public double getRxPowerLevel_mW(JE802Phy phy) {
        return getRxPowerLevel_mW((afPhy) phy) ;
                }

	public double getRxPowerLevel_mW(afPhy phy) {
		...
		if (channel == null) {
			...
			for (JE802MediumTxRecord rec : ownTransmssions) {
				...
				double attenuation = palo.getPathloss((rec.getTxPhy()).getMac().getMacAddress(), phy.getMac().getMacAddress(), this);
    ...
    public double getSnirAtRx(int rxAddr, JE802Phy txPhy) {
        return getSnirAtRx(rxAddr, (afPhy) txPhy) ;
    }
    
	public double getSnirAtRx(int rxAddr, afPhy txPhy) {
