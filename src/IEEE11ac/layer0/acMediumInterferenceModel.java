...
public class acMediumInterferenceModel extends JEEventHandler implements WirelessMedium {
	...
	final List<acPhy> allPhys;

	// lookup table for attenuation factors
        // key, (key, value)
        // station, (station, attenuation)
        
        public acAttenuationTable attnTable ;

	final Map<Integer, WirelessChannel> availableChannels;
        
        public acPathLoss palo = new acPathLoss() ;

	public acMediumInterferenceModel(final JEEventScheduler aScheduler, final Random aGenerator, final Node aTopLevelNode) {
...
//		acAttenuationTable = new HashMap<Integer, Map<Integer, Double>>();
                attnTable = new acAttenuationTable();
                
		allPhys = new ArrayList<acPhy>();
	...
	private void transmitStart(final JEEvent anEvent) {
		...
		acPhy aTxPhy = (acPhy) anEvent.getParameterList().elementAt(0);
		...
		Map<Integer, Double> neighbors = acAttenuationTable.get(aTxPhy.getMac().getMacAddress());
		...
		List<acPhy> channelPhys = chan.getPhyList();
		int size = channelPhys.size();
		for (int i = 0; i < size; i++) {
			acPhy rxPhy = channelPhys.get(i);
	...
	private void transmitEnd(final JEEvent anEvent) {
		...
		for (Integer rxStation : transmission.getRxStations()) {
			...
			JE802Phy rxPhy = null;
			// search for rxPhy
			for (acPhy aRxPhy : channel.getPhyList()) {
	...
	private void switchChannel(JEEvent anEvent) {
		acPhy phy = (acPhy) anEvent.getParameterList().get(0);
	...
	private double calculateInterference(final acPhy atPhy, final JE802Phy txPhy, final JE802ChannelEntry chan) {
	...
	private void registerPhy(final JEEvent anEvent) {
		acPhy newPhy = (acPhy) anEvent.getParameterList().elementAt(0);
	...
	private void locationUpdate(JEEvent anEvent) {
		acPhy updatePhy = (acPhy) (JE802Phy) anEvent.getParameterList().get(0);
	...
	protected class JE802ChannelEntry {
		...
		private final List<acPhy> phyList;
		...
		public List<acPhy> getPhyList() {
			return phyList;
		}

		public void addPhy(final acPhy phy) {
			phyList.add(phy);
			for (JE802MediumTxRecord rec : transmittingPhys) {
		...
		public JE802MediumTxRecord addTransmittingPhy(final acPhy txPhy, final JETime txEnd) {
			JE802MediumTxRecord txRec = new JE802MediumTxRecord(txPhy, txEnd);
			Map<Integer, Double> neighbors = acAttenuationTable.get(txPhy.getMac().getMacAddress());
			Set<Integer> rxStations = new HashSet<Integer>();
			for (acPhy addr : phyList) {
	...
	public class JE802MediumTxRecord {
		...
		public void addRxStation(acPhy phy, double interference) {
    ...
    public double getRxPowerLevel_mW(JE802Phy phy) {
        return getRxPowerLevel_mW((acPhy) phy) ;
                }

	public double getRxPowerLevel_mW(acPhy phy) {
		...
		if (channel == null) {
			...
			for (JE802MediumTxRecord rec : ownTransmssions) {
				...
				double attenuation = palo.getPathloss(((acPhy) rec.getTxPhy()).getMac().getMacAddress(), phy.getMac().getMacAddress(), this);
    ...
    public double getSnirAtRx(int rxAddr, JE802Phy txPhy) {
        return getSnirAtRx(rxAddr, (acPhy) txPhy) ;
    }
    
	public double getSnirAtRx(int rxAddr, acPhy txPhy) {
		double ownChannelInterference = noiseLevel_mW;
		double attenuation = palo.getPathloss(txPhy.getMac().getMacAddress(), rxAddr, this);
