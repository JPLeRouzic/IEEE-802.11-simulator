...
import IEEE11ac.layer1.acAttenuationTable;
...
public class acPathLoss {
    ...
    void updateAttenuations(acPhy newPhy, acMediumInterferenceModel mim) {
		acMediumInterferenceModel.JE802ChannelEntry entry = mim.channels.get(newPhy.getCurrentChannelNumberTX());
		Map<Integer, Double> attenuations = acAttenuationTable.get(newPhy.getMac().getMacAddress());
		if (attenuations == null) {
			attenuations = new HashMap<Integer, Double>();
		}
		for (acPhy otherPhy : mim.allPhys) {
			...
			Map<Integer, Double> othersAttenuations = acAttenuationTable.get(otherPhy.getMac().getMacAddress());
    ...
    double getPathloss(final Integer sta1, final Integer sta2, acMediumInterferenceModel mim) {
		Map<Integer, Double> mapPhy1 = acAttenuationTable.get(sta1);
    ...
    public double getLinkRxPowerLevel_mW(acPhy locphy, acPhy dstphy, acMediumInterferenceModel mim) {
        acMediumInterferenceModel.JE802ChannelEntry channel = mim.channels.get(locphy.getCurrentChannelNumberTX());
        if (channel == null) {
    ...
    private double computePathlossFactor(final JE802Phy srcPhy, final JE802Phy dstPhy, final double p1m, acMediumInterferenceModel mim) {
...
//	@Override
	public double getReuseDistance(acPhy locphy, acPhy dstphy, acMediumInterferenceModel mim) {
		double distance ;
                double rxpwr, rxdb ;
...
//	@Override
	public double getBusyPowerLevel_mW(acMediumInterferenceModel mim) {
		return mim.busyThreshold_mW;
	}

//	@Override
	public double getGlobalRxPowerLevel_mW(acPhy phy, acMediumInterferenceModel mim) {
		double neighborChannelInterference = 0.0;
		acMediumInterferenceModel.JE802ChannelEntry channel = mim.channels.get(phy.getCurrentChannelNumberTX());
		if (channel == null) {
			...
			return 0.0;
                    ...
                    for (Integer interferingChannel : channel.getInterferingChannels()) 
                        {
                        acMediumInterferenceModel.JE802ChannelEntry entry = mim.channels.get(interferingChannel);
                        List<acMediumInterferenceModel.JE802MediumTxRecord> neighborTransmissions = entry.getTransmittingPhys();
                        for (acMediumInterferenceModel.JE802MediumTxRecord rec : neighborTransmissions) 
                            {
                            acPhy txPhy = (acPhy) rec.getTxPhy();
                    ...
                    List<acMediumInterferenceModel.JE802MediumTxRecord> ownTransmssions = channel.getTransmittingPhys();
                    double ownChannelInterference = 0.0;
                    for (acMediumInterferenceModel.JE802MediumTxRecord rec : ownTransmssions) 
                        {
...
//	@Override
	...
	public double getSinrAtRx(Integer rxAddr, acPhy txPhy, acMediumInterferenceModel mim) {
    ...
    public double calculateInterference(final acPhy atPhy, final JE802Phy txPhy, final acMediumInterferenceModel.JE802ChannelEntry chan, acMediumInterferenceModel jE802MediumInterferenceModel) {
        double interferenceSummW = 0.0;
        List<acMediumInterferenceModel.JE802MediumTxRecord> physOnChannel = chan.getTransmittingPhys();
        int size = physOnChannel.size();
        for (int i = 0; i < size; i++) {
            acMediumInterferenceModel.JE802MediumTxRecord otherTx = physOnChannel.get(i);
            acPhy currentPhy = (acPhy) otherTx.getTxPhy();
            if (currentPhy != txPhy) {
    ...
    public void addInterference(final acPhy aTxPhy, final double crossChannelInterference, acMediumInterferenceModel mim, JE802Phy txPhy) {
        if (aTxPhy != txPhy) {
    ...
    public void decreaseInterference(final acPhy aTxPhy, final double crossChannelInterference, acMediumInterferenceModel mim, JE802Phy txPhy) {
        if (aTxPhy != txPhy) {
            for (Integer rxAddr : rxInterferencemW.keySet()) {
                double attenuation = getPathloss(aTxPhy.getMac().getMacAddress(), rxAddr, mim);
                double interferenceAtRx = aTxPhy.getCurrentTransmitPower_mW() * attenuation * crossChannelInterference;
    ...
    public void addRxInterference(final acPhy aRxPhy, final double interferencemW) {
