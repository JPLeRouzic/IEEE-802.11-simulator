...
public class JE802RouteLinksKml extends JE802LinkKmlGenerator {

	private Map<Integer, JE802Station> stationMap;

	private List<JE802RouteHopRecord> linkRecords;

	public JE802RouteLinksKml(Document doc, List<JE802Station> stations) {
		super(doc, stations);
		...
		for (Integer sa : routes.keySet()) {
			...
			for (Integer destination : destinations.keySet()) {
				...
				Color routeColor = new Color((int) (Math.random() * 255), (int) (Math.random() * 255),
						(int) (Math.random() * 255));
					...
					for (int i = 1; i < hopList.size(); i++) {
						...
						for (int j = 0; j < timeSteps; j++) {
							...
							JE802RouteHopRecord rec = new JE802RouteHopRecord(hopSource.getMacAddress(),
									hopDestination.getMacAddress(), currentTime, currentTime.plus(interval), routeColor, sa,
									destination);
							linkRecords.add(rec);
						}
	...
	public Element createDOM() {
		...
		return createFolder(routeFolders, "Route Links", false);
	}
}
