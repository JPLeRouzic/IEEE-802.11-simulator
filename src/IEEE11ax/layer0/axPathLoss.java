...
public class axPathLoss {
    ...
    void updateAttenuations(axPhy newPhy, axMediumInterferenceModel mim) {
		axMediumInterferenceModel.JE802ChannelEntry entry = mim.channels.get(newPhy.getCurrentChannelNumberTX());
		Map<Integer, Double> attenuations = axAttenuationTable.get(newPhy.getMac().getMacAddress());
		if (attenuations == null) {
			attenuations = new HashMap<Integer, Double>();
		}
		for (axPhy otherPhy : mim.allPhys) {
			...
			Map<Integer, Double> othersAttenuations = axAttenuationTable.get(otherPhy.getMac().getMacAddress());
    ...
    double getPathloss(final Integer sta1, final Integer sta2, axMediumInterferenceModel mim) {
		Map<Integer, Double> mapPhy1 = axAttenuationTable.get(sta1);
    ...
    public double getLinkRxPowerLevel_mW(axPhy locphy, axPhy dstphy, axMediumInterferenceModel mim) {
        axMediumInterferenceModel.JE802ChannelEntry channel = mim.channels.get(locphy.getCurrentChannelNumberTX());
        if (channel == null) {
    ...
    private double computePathlossFactor(final JE802Phy srcPhy, final JE802Phy dstPhy, final double p1m, axMediumInterferenceModel mim) {
...
//	@Override
	public double getReuseDistance(axPhy locphy, axPhy dstphy, axMediumInterferenceModel mim) {
		double distance ;
                double rxpwr, rxdb ;
...
//	@Override
	public double getBusyPowerLevel_mW(axMediumInterferenceModel mim) {
		return mim.busyThreshold_mW;
	}

//	@Override
	public double getGlobalRxPowerLevel_mW(axPhy phy, axMediumInterferenceModel mim) {
		double neighborChannelInterference = 0.0;
		axMediumInterferenceModel.JE802ChannelEntry channel = mim.channels.get(phy.getCurrentChannelNumberTX());
		if (channel == null) {
			...
			return 0.0;
                    ...
                    for (Integer interferingChannel : channel.getInterferingChannels()) 
                        {
                        axMediumInterferenceModel.JE802ChannelEntry entry = mim.channels.get(interferingChannel);
                        List<axMediumInterferenceModel.JE802MediumTxRecord> neighborTransmissions = entry.getTransmittingPhys();
                        for (axMediumInterferenceModel.JE802MediumTxRecord rec : neighborTransmissions) 
                            {
                    ...
                    List<axMediumInterferenceModel.JE802MediumTxRecord> ownTransmssions = channel.getTransmittingPhys();
                    double ownChannelInterference = 0.0;
                    for (axMediumInterferenceModel.JE802MediumTxRecord rec : ownTransmssions) 
                        {
...
//	@Override
	...
	public double getSinrAtRx(Integer rxAddr, axPhy txPhy, axMediumInterferenceModel mim) {
    ...
    public double calculateInterference(final axPhy atPhy, final JE802Phy txPhy, final axMediumInterferenceModel.JE802ChannelEntry chan, axMediumInterferenceModel jE802MediumInterferenceModel) {
        double interferenceSummW = 0.0;
        List<axMediumInterferenceModel.JE802MediumTxRecord> physOnChannel = chan.getTransmittingPhys();
        int size = physOnChannel.size();
        for (int i = 0; i < size; i++) {
            axMediumInterferenceModel.JE802MediumTxRecord otherTx = physOnChannel.get(i);
    ...
    public void addInterference(final axPhy aTxPhy, final double crossChannelInterference, axMediumInterferenceModel mim, JE802Phy txPhy) {
        if (aTxPhy != txPhy) {
    ...
    public void decreaseInterference(final axPhy aTxPhy, final double crossChannelInterference, axMediumInterferenceModel mim, JE802Phy txPhy) {
        if (aTxPhy != txPhy) {
            for (Integer rxAddr : rxInterferencemW.keySet()) {
                double attenuation = getPathloss(aTxPhy.getMac().getMacAddress(), rxAddr, mim);
                double interferenceAtRx = aTxPhy.getCurrentTransmitPower_mW() * attenuation * crossChannelInterference;
    ...
    public void addRxInterference(final axPhy aRxPhy, final double interferencemW) {
